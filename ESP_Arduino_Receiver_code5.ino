...
void ESP_post()
{
  ...
  if (stat.substring(0) == "L1=0")  // Light_1
  {
    ...
    send_to_main = "";
    data = "";
    data = F("GET /update?api_key=VZGWTTS9IC63O7AG;&field1=0");  // light_1
    ...
    //data = F("GET /update?api_key=VZGWTTS9IC63O7AG;&field1=0");  // using F() so that the string initially is stored only in ROM and not copied in RAM
...
bool ESP_initialise()
{
  ...
  ESP.println (F("AT+CIOBAUD=74880"));
  delay(1000);
  if (ESP.find("OK"))
    Serial.println(F("Baud rate = 74880"));
  else
